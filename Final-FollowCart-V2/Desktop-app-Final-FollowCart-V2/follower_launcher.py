...
class FollowerSystemLauncher:
    def __init__(self, root):
        ...
        self.root.geometry("600x700")  # Increased height to accommodate team info
    ...
    def launch_system(self):
        ...
        try:
            ...
            # Launch the selected script with the COM port as argument
            subprocess.Popen([sys.executable, script_name, com_port])
            self.info_label.config(
                text=f"Successfully launched:\n{system_type}\n\nCOM Port: {com_port}",
                fg='#27ae60'
            )
            
            # Reset button after short delay
            self.root.after(2000, lambda: self.launch_btn.config(text="LAUNCH SYSTEM", state=tk.NORMAL))
        except Exception as e:
            messagebox.showerror("Error", f"Failed to launch system:\n{str(e)}")
            self.launch_btn.config(text="LAUNCH SYSTEM", state=tk.NORMAL)
            self.info_label.config(text=f"Error: {str(e)}", fg='#e74c3c')
